...
import { ShieldCheck, LogIn, HardHat, CheckCircle2, Phone, Key, Loader2, Lock, Smartphone, UserCircle, ArrowRight, UserPlus, History, UserMinus, Zap } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isRegistering) {
        ...
        userDb[phone] = newUser;
        localStorage.setItem('intelliplan_user_db', JSON.stringify(userDb));
        targetUser = newUser; // 注册成功后自动登录
    } else {
        // 登录流程
        if (!targetUser) {
            // 特殊处理：如果是管理员手机号但未在DB中（初始化情况），允许直接通过密码登录
            if (phone === ADMIN_PHONE) {
                if (loginMethod === 'password' && password === DEFAULT_ADMIN_PWD) {
                    targetUser = {
                        id: phone,
                        username: '系统管理员',
                        phone: phone,
                    ...
                    // 自动补全DB
                    userDb[phone] = targetUser;
                    localStorage.setItem('intelliplan_user_db', JSON.stringify(userDb));
                } else {
                     setError('管理员账号未初始化或密码错误');
                     return;
                }
            } else {
                setError('账号不存在，请先注册');
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-100/40 backdrop-blur-md overflow-hidden">
      ...
      <div className="relative w-full max-w-md p-10 bg-white/70 backdrop-blur-3xl border border-white/60 rounded-[3rem] shadow-[0_40px_100px_rgba(15,23,42,0.1)] fade-in-up">
        ...
        {lastUser ? (
          <div className="mb-4 space-y-4 animate-in fade-in zoom-in duration-500">
          ...
          <>
            {/* Toggle Switch */}
            <div className="flex bg-slate-100/50 p-1.5 rounded-2xl border border-slate-200/50 mb-6">
              <button 
                onClick={() => { setIsRegistering(false); setError(''); }}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-black rounded-xl transition-all ${!isRegistering ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
              >
                <LogIn size={15} /> 账号登录
              </button>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-2">手机号</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3.5 text-slate-900 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold placeholder:text-slate-300 shadow-inner"
              ...
              {isRegistering && (
                <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                   <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-2">短信验证码</label>
                    <div className="flex gap-3">
                        ...
                        className="px-4 bg-slate-900 text-white text-xs font-black rounded-2xl transition-all disabled:opacity-50 min-w-[90px] active:scale-95 shadow-lg"
                        >
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2 ml-2">设置昵称</label>
                    <div className="relative">
                        <UserCircle size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-300" />
                        <input 
                        type="text" 
                        value={username}
                        onChange={(e) => setUsername(e.target.value)}
                        className="w-full bg-emerald-50/30 border border-emerald-100 rounded-2xl pl-12 pr-4 py-3.5 text-slate-900 outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all font-bold placeholder:text-emerald-300/50"
                        placeholder="例如：张工"
                        required
                        />
                    </div>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2 ml-2">设置登录密码</label>
                    <div className="relative">
                        <Lock size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-300" />
                        <input 
                        type="password" 
                        value={password}
                ...
                <div className="animate-in slide-in-from-top-2 duration-300 space-y-4">
                  <div className="flex gap-2 mb-2 px-2">
                     <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="loginType" className="accent-blue-600" checked={loginMethod === 'password'} onChange={() => setLoginMethod('password')} />
                        <span className="text-[10px] font-bold text-slate-600">密码登录</span>
                     </label>
                     <label className="flex items-center gap-2 cursor-pointer ml-4">
                        <input type="radio" name="loginType" className="accent-blue-600" checked={loginMethod === 'code'} onChange={() => setLoginMethod('code')} />
                        <span className="text-[10px] font-bold text-slate-600">验证码登录</span>
                     </label>
                  </div>
              ...
              <div className="flex items-center justify-between px-2 pt-2">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <div className={`w-4 h-4 rounded-lg border-2 flex items-center justify-center transition-all ${rememberMe ? 'bg-blue-600 border-blue-600 shadow-lg shadow-blue-500/20' : 'border-slate-200 bg-white group-hover:border-slate-300'}`} onClick={() => setRememberMe(!rememberMe)}>
                    {rememberMe && <CheckCircle2 size={10} className="text-white" />}
                  </div>
                  <span className="text-[10px] font-bold text-slate-500 select-none">记住登录</span>
                </label>
                {!isRegistering && <button type="button" className="text-[10px] font-bold text-blue-600 hover:text-blue-500 transition-colors">忘记密码？</button>}
              </div>

              {error && <p className="text-red-500 text-xs font-black text-center animate-bounce py-1">{error}</p>}

              <button 
                type="submit" 
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-[1.5rem] shadow-xl shadow-blue-500/30 transition-all active:scale-[0.97] flex justify-center items-center gap-3 mt-4"
              >
                {isRegistering ? <UserPlus size={20} /> : <LogIn size={20} />}
                <span className="text-sm tracking-wide">{isRegistering ? '立即注册' : '进入系统'}</span>
              </button>
            </form>
          </>
        )}
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col items-center">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 text-[10px] text-slate-400 font-black tracking-widest uppercase">
              <CheckCircle2 size={14} className="text-emerald-500" /> 安全审计
            </div>
          </div>
        </div>
      </div>

      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] opacity-80">
        © 2024 IntelliPlan Engineering
      </div>
    </div>
  );
};
